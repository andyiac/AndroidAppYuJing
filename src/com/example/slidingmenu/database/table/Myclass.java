...
public class Myclass implements TableCreateInterface {
	...
	//建立数据表
		...
		public void onCreate(SQLiteDatabase db){
			...
			String sql = "CREATE TABLE "
					+ Myclass.tableName
					+ " (  "
					+ Myclass._id + " integer primary key,"				
					+ Myclass.cname + " TEXT)";
			db.execSQL( sql );
		}
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			if ( oldVersion < newVersion ) {
				String sql = "DROP TABLE IF EXISTS " + Myclass.tableName;
				db.execSQL( sql );
				this.onCreate( db );
			}
		}
		...
		public static void deleteClass(AttendanceHelper attendhelper ,int id) {
			SQLiteDatabase db = attendhelper.getWritableDatabase();
			db.delete(tableName, Myclass._id + "=?",new String[] { id + "" }  );
			db.close();
		}
		...
		public static void updateClass(AttendanceHelper attendhelper, String name,int id) {
			SQLiteDatabase db = attendhelper.getWritableDatabase();
			ContentValues contentValues = new ContentValues();
			contentValues.put(Myclass.cname, name );
		...
		public static String getClassName(AttendanceHelper attendhelper,int id) {
			...
			Cursor cursor = db.query(tableName, new String[] {_id,cname }, "_id = ?", new String[]{id+""},
					null, null, null);
			cursor.moveToFirst();
			String name =cursor.getString(cursor.getColumnIndex(Myclass.cname));
			return name;
		}
