...
public class SignProfileActivity_bak extends Activity{
...
//		rs.setPassword("");
	...
	private int signProfile(int array, int title, final TextView show) {
		...
		Builder builder = new Builder(SignProfileActivity_bak.this);
	...
	public class SigninTask extends AsyncTask<RequestParam, Integer, Integer>{
		...
		protected void onPreExecute() {
			dialog = ProgressDialog.show(SignProfileActivity_bak.this, "", getText(R.string.waiting));
			super.onPreExecute();
		}
		@Override
		protected Integer doInBackground(RequestParam... params) {
			
			if(!HttpClient.isConnect(SignProfileActivity_bak.this)) {
				return -1;
			}
		...
		protected void onPostExecute(Integer result) {
			...
			dialog.dismiss();
			if(result == 0) {
				Toast.makeText(SignProfileActivity_bak.this, "signin succ", Toast.LENGTH_SHORT).show();
				finish();
			} else {
				Toast.makeText(SignProfileActivity_bak.this, "signin fail", Toast.LENGTH_SHORT).show();
			}
		}
